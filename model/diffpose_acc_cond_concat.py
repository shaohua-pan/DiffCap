...
class DiffPoseAccCondConcat(nn.Module):
    def __init__(self, njoints, nfeats, acc_dim,
        ...
        self.embed_input = nn.Linear(self.input_feats + self.acc_dim, self.latent_dim)
    ...
    def forward(self, x, timesteps, y=None):
        ...
        # d2_emb = self.embed_acc(y['acc'])
        # emb += self.mask_cond(d2_emb)

        cond = self.mask_cond(y['acc']).reshape(bs*nframes, self.acc_dim)
        x = x.permute(0, 3, 1, 2).reshape(bs*nframes, njoints*nfeats)
        x = torch.cat((x, cond), dim=-1)
        x = self.embed_input(x).reshape(bs, nframes, -1).permute(1, 0, 2)  # [seqlen, bs, d]
