...
class DiffusionPoseAttn(nn.Module):
    def __init__(self, njoints, nfeats, imu_3d_dim, mp_dim,
        ...
        self.embed_condition = nn.Linear(self.imu_3d_dim, self.latent_dim)
    ...
    def forward(self, x, timesteps, y=None):
        ...
        t = self.embed_timestep(timesteps)  # [1, bs, d]
        ...
        cond = self.embed_condition(imu_3d).reshape(bs, nframes, -1).permute(1, 0, 2)  #[nframes, bs, d]
        xseq = torch.cat((t, cond, x), dim=0)  # [2*nframes+1, bs, d]
        xseq = self.sequence_pos_encoder(xseq)  # [nframes+1, bs, d]
        output = self.seqTransEncoder(xseq)[-nframes:]  # [nframes, bs, d]
