...
class Diffusion3dExchangeInputCond(nn.Module):
    def __init__(self, njoints, nfeats, imu_dim, mp_dim,
        ...
        self.embed_input = nn.Linear(self.input_feats + self.mp_dim, self.latent_dim)
        ...
        self.embed_imu = nn.Linear(self.imu_dim, self.latent_dim)
    ...
    def forward(self, x, timesteps, y=None):
        ...
        imu_emb = self.mask_cond(self.embed_imu(y['imu']))
        emb += imu_emb

        x = x.permute(0, 3, 1, 2).reshape(bs*nframes, njoints*nfeats)  # [bs*nframes, njoints*nfeats]
        mp = y['mp'].reshape(bs*nframes, self.mp_dim)
        x = torch.cat((x, mp), dim=1)  # [bs*nframes, njoints*nfeats+mp_imu_dim]
