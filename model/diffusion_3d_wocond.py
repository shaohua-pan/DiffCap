...
class Diffusion3dWoCond(nn.Module):
    def __init__(self, njoints, nfeats, imu_dim, mp_dim,
        ...
        self.embed_input = nn.Linear(self.input_feats + self.imu_dim + self.mp_dim, self.latent_dim)
    ...
    def forward(self, x, timesteps, y=None):
        ...
        # mp_emb = self.mask_cond(self.embed_mp(y['mp']))
        # emb += mp_emb

        x = x.permute(0, 3, 1, 2).reshape(bs*nframes, njoints*nfeats)  # [bs*nframes, njoints*nfeats]
        imue = y['imu'].reshape(bs*nframes, self.imu_dim)
        mpe = y['mp'].reshape(bs*nframes, self.mp_dim)
        x = torch.cat((x, mpe, imue), dim=1)
