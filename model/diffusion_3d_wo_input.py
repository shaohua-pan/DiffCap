...
class Diffusion3dWoInput(nn.Module):
    def __init__(self, njoints, nfeats, mp_imu_dim,
        ...
        self.embed_mp_imu = nn.Linear(self.mp_imu_dim, self.latent_dim)
    ...
    def forward(self, x, timesteps, y=None):
        ...
        mp_emb = self.mask_cond(self.embed_mp_imu(y['mp_imu']))
        emb += mp_emb

        x = x.permute(0, 3, 1, 2).reshape(bs*nframes, njoints*nfeats)  # [bs*nframes, njoints*nfeats]
        # mp_imu = y['imu'].reshape(bs*nframes, self.imu_dim)
        # x = torch.cat((x, mp_imu), dim=1)  # [bs*nframes, njoints*nfeats+mp_imu_dim]
